...
namespace Mountain.classes.helpers {
    ...
    public class ApplicationSettings {
        ...
        public void InitializeSettings() {
            ...
            if (!Directory.Exists(RoomBuildDirectory)) {
                Directory.CreateDirectory(RoomBuildDirectory);
            }
            if (!Directory.Exists(WorldBuildDirectory)) {
                Directory.CreateDirectory(WorldBuildDirectory);
            }
            if (!Directory.Exists(AreaBuildDirectory)) {
                Directory.CreateDirectory(AreaBuildDirectory);
            }
            if (!Directory.Exists(ExitBuildDirectory)) {
                Directory.CreateDirectory(ExitBuildDirectory);
            }
            if (!Directory.Exists(TemplateDirectory)) {
                Directory.CreateDirectory(TemplateDirectory);
            }
            if (!Directory.Exists(WorldTemplateDirectory)) {
                Directory.CreateDirectory(WorldTemplateDirectory);
            }
            if (!Directory.Exists(AreaTemplateDirectory)) {
                Directory.CreateDirectory(AreaTemplateDirectory);
            }
            if (!Directory.Exists(RoomTemplateDirectory)) {
                Directory.CreateDirectory(RoomTemplateDirectory);
            }
            if (!Directory.Exists(ExitTemplateDirectory)) {
                Directory.CreateDirectory(ExitTemplateDirectory);
            }
            if (!Directory.Exists(ItemTemplateDirectory)) {
                Directory.CreateDirectory(ItemTemplateDirectory);
            }
