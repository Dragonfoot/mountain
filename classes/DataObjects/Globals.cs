...
namespace Mountain.classes.dataobjects {
    ...
    [Flags] public enum roomType {
        empty = 0, path = 1, street = 2, road = 4, sewer = 8, space = 16, water = 32, plains = 64, forest = 128,
        mountain = 256, underground = 512, indoor = 1024, outdoor = 2048, administrator = 4096, home = 8192
    }
    ...
    public enum exitType {
        door, window, directional, random, teleporter
    }
    public enum lockType {
        key, pin, owner, spell, skill, item, password
    }
    [Flags] public enum restrictionType {
        level, administration, race, character, size, magic, quest
    }
    public enum direction {
        east, north, west, south, down, up
    }
